...
const Posts = ({ postDetails, user, history }) => {
  ...
  useEffect(() => {
    if (!socket.current) {
      socket.current = io("http://127.0.0.1:4000")
    }
    setPost(postDetails)
  }, [])
      ...
      <Row className='mt-5 mb-3 text-center'>
        ...
        {post.user && (
          <>
            ...
            <Col md={4} onClick={redirectHandler}>
              <i className='far fa-comment-alt h5 grey-color'> ({post.comments.length})</i>
            </Col>

            <Col md={4}>
              {user && post.bookmarks.includes(user._id.toString()) ? <i className='fas fa-bookmark gray-bookmark' onClick={postBookmarkHandler}> Bookmark</i> : <i className='far fa-bookmark h5 grey-color' onClick={postBookmarkHandler}> Bookmark</i>}
            </Col>
          </>
        )}
